...
``ver2/video/weights``. That directory has never existed, so `weight_path` fell
...
``REPO_ROOT``   the checkout. Everything below is anchored to it rather than to
                the working directory, so `python -m ver2.…` behaves the same
...
#: The checkout: this file is `<root>/ver2/paths.py`.
