...
class TextRegionDescriptor(RegionDescriptor):
    """Position decides what to compare; a layout hash decides if it changed.

    Text is the case where geometry alone fails. A presentation advances a
    slide and the text block does not move an inch — identical boxes, wholly
    different content. So overlap is used only as a gate to decide which
    regions are candidates for being "the same block", and the comparison
    itself is a hash of the pixel layout inside.

    Regions that overlap nothing in the reference score zero, which is what
    makes new text anywhere in frame register as a change.
    """

    name = "text_region"

    def __init__(self, grid: int = 24, iou_gate: float = 0.3) -> None:
        self.grid = grid
    ...
    def describe(self, image: np.ndarray, detections: Sequence[Detection]) -> dict:
        boxes, layouts = [], []
        for d in detections:
            crop = d.crop(image, pad=0.0)
            if crop is None:
                continue
            boxes.append([d.x1, d.y1, d.x2, d.y2])
            layouts.append(_layout_vector(crop, self.grid))
        if not boxes:
            return {
                "boxes": np.zeros((0, 4)),
                "layouts": np.zeros((0, self.grid * self.grid), dtype=np.float32),
            }
        return {"boxes": np.array(boxes, dtype=np.float64), "layouts": np.array(layouts)}

    def count(self, described: dict) -> int:
        return len(described["boxes"])

    def similarity(self, current: dict, reference: dict) -> np.ndarray:
        overlap = _iou_matrix(current["boxes"], reference["boxes"])
        if overlap.size == 0:
            return overlap
        # Vectors are centred and unit-norm, so this is a correlation.
        agree = np.clip(current["layouts"] @ reference["layouts"].T, 0.0, 1.0)
        # Regions that do not overlap cannot be the same block of text,
        # however similar their contents happen to look.
        return np.where(overlap >= self.iou_gate, agree, 0.0)

    def config(self) -> dict:
        return {"name": self.name, "grid": self.grid, "iou_gate": self.iou_gate}
