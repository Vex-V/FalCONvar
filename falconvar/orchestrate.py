...
This is what `falconvar/driver.py` used to hold inline, extracted because it now has
...
from falconvar.audio.output import (MultiTranscriptSink, TranscriptDocument,
                               build_document)
from falconvar.audio.reader import cut, listen
from falconvar.timeline import Timeline
from falconvar.timeline import uniform as timeline_uniform
from falconvar.video.ingest.output import FileManifestWriter, FrameStore, MultiSink
from falconvar.video.ingest.pipeline import ingest
...
def _video_duration(media: Path) -> Optional[float]:
    ...
    from falconvar.video.ingest.source import probe as video_probe
...
def _manifest_sinks(names: Sequence[str], out_dir: Path):
    ...
    for name in names:
        if name == "file":
            ...
            from falconvar.video.ingest.output import SupabaseManifestWriter
