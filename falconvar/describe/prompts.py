...
Data, not logic: instructions, response schemas, and which keys each question
owns. Nothing here imports anything.

Two kinds of question. **The scene question is general** -- setting, people,
objects, visible_text, actions, changes, tags -- and is the fallback. **A
specialist question is narrow and structured**, returning one object per entity
rather than parallel lists, because a list of people beside a list of actions
does not say who did what.

Which keys the scene question may fill depends on which other questions are
being asked about the same chunk: a key a specialist owns is removed from the
scene schema, so ownership is exclusive per chunk and merging reconciles
nothing.

Any sampler may be paired with any question here. `QUESTIONS` is the vocabulary
and is validated before a run rather than fallen through -- an unknown name
would otherwise quietly get the scene question and bill for it.
"""
...
def questions_on(manifest, chunk: dict[str, Any]) -> list[str]:
    ...
    Questions, never sampler ids. `OWNER` is keyed by question, and the two are
    only equal while no sampler is paired with someone else's question. With
    `yolo:overview` on a chunk, passing ids would have `clip` give up `people`
    to a call that was asked `overview` and owns no keys at all -- the field
    would leave the document with everything still well-formed.
    """
