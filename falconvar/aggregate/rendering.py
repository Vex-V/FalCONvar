...
`summary`, `chapters` and `events` all need the same thing: the video as lines
of text, one per chunk, in order, with the chunk ids attached. Written once so
three aggregators cannot format it differently and then disagree about which
chunk a sentence came from.

Every account of a chunk goes in, labelled, not the first one found: on a
narrated video the picture and the soundtrack are two independent accounts of
the same seconds.

Ids travel with the text, so a merged span is the union of what it merged.
Spans are resolved through the timeline rather than trusted from the model --
it is asked for chunk ids, which it can copy; times it would invent.
"""
