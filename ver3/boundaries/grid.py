"""4 · the grid -- evidence plus a duration, into spans.

The only component that decides boundaries. Everything else either produces
evidence for it (`scenes`, `speech`) or reads the grid it wrote.

**`uniform` needs no evidence at all**, and that is worth noticing: it is
arithmetic over the container duration, which `media.json` already recorded. It
therefore needs neither modality to have run. `falconvar` computes it inside
the video pass and reads it back afterwards, decoding 5132 frames to learn a
number that was in the container header; and separately computes the same grid
from the track duration on an audio-only run, so one policy has two
implementations that must agree.

**Both guards are mandatory for a content-derived policy**, in both directions.
Voice activity cuts on every pause -- every second or two on conversational
audio -- which would shred the video into chunks too short to describe and
multiply the describer calls that dominate cost. A single unbroken monologue
produces zero cuts and one chunk covering the file, which is no grid at all.
`min_s` merges; `max_s` splits.

`max_s` splits **evenly**, not into fixed bites. Taking 30 s bites off a 62.5 s
span leaves a 2.5 s remainder, so a guard whose whole job is preventing tiny
chunks would create one. It becomes three of 20.8 s instead.
"""
...
from ..shared.documents import Timeline
