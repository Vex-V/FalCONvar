...
**This is the file the whole cost argument turns on.** Decoding a frame costs
0.40 ms; converting one to a full-resolution BGR array costs 6.2 ms. Ingest
needs pixels only for *decimated* frames -- one a second out of twenty-five, so
4% of them -- and everything else can be decided from `media_ts` alone, which
is `pts * time_base` and needs no pixels at all.

So the decimator's verdict is asked **before** the conversion, and 96% of
frames never become an array:

    3 h of 720p25, converting every frame        37 min
    3 h of 720p25, converting only decimated      3 min

`falconvar` converts unconditionally inside its own `read_frames`, because its
scene chunker needs pixels at native rate and the reader is the only thing
holding them. With scene detection moved to its own pass there is nothing left
that wants a frame ingest is not going to sample, and the 12x goes away.

Sequential rather than seeking, and that is not a simplification: frames
reference each other, so producing the frame at second 47 means decoding
forward from its keyframe regardless. Seeking to sample would cost *more*
decode work, not less.

A generator, so decoding happens between iterations of the caller's loop rather
than before it. Exactly one frame is in flight -- the alternative is 4485
frames at ~6 MB for a five-minute 1080p file.
"""
