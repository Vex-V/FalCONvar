"""One decode pass, feeding every sampler from it.
    ...
    read -> decimate -> which chunk -> sampler(s) -> release
...
**The grid is an input and is never edited.** Ingest asks
`timeline.nearest(media_ts)` and that is the whole of its relationship with
boundaries. It does not derive them, does not correct a final `end_ts`, and
does not merge a short tail -- all three exist in `falconvar` only because the
pass there produces the grid it is simultaneously consuming, so decisions get
made before the information to make them exists and have to be patched
afterwards. Here the grid was decided by `boundaries`, from evidence, before
this ran.

That removes the last thing that made a `Chunker` protocol necessary: nothing
in this file needs `observe()` at native rate, an open-ended `bounds_of`, or a
correction pass.
"""
