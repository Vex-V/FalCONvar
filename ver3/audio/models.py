...
One file rather than two packages of four files. `falconvar` gives
`transcribe/` and `diarize/` a `base.py`, a registry `__init__.py` and one
module per backend -- eight files to express "there are two of these, and both
are swappable". The protocols are four lines each and the registries are a
dict; the only thing that genuinely needs isolating is the heavy import, and a
lazy registry does that in one line per entry.

**Nothing here is imported until it is asked for.** `faster_whisper` and
`pyannote` between them pull in torch, and a `--transcriber stub` run should
not pay for either. Same discipline the sampler registry uses on the video
side: the entry is a string until `build` resolves it.
"""
