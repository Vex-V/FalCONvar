"""Generating JSON Schema from the document dataclasses.

The dataclasses in `documents.py` are the single source of truth. These files
are generated from them and checked in, and CI regenerates and fails on a
non-empty diff -- so the three descriptions of a document (dataclass, JSON
Schema, SQL DDL) cannot drift apart silently.

**Why generate rather than author.** A hand-written schema beside a dataclass
is two statements of one fact, and the pair is only correct until someone edits
one. `falconvar` has the same shape of problem in its database: `add column if
not exists fts` is a no-op when the column exists, so a generated column built
by an older version of the file survived a re-run untouched, and the lexical
half quietly stopped indexing the terms it is best at. Nothing reported it.

**Validation is partial on purpose.** The header and structure are checked on
every read; the per-frame arrays only under a flag. A three-hour manifest holds
tens of thousands of frame records, and validating each on every read would
make the contract layer a tax on the hot path.
"""
