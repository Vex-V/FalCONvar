...
def api_key(explicit: Optional[str] = None) -> str:
    """The key, or a message naming every place it could have come from.

    Loads `.env` itself rather than trusting a caller to have done it. The
    embedder did not, and failed on a key that was present -- the module that
    owns the key names is the one that should guarantee it is readable.
    """
    if explicit:
        return explicit
    env.load()
