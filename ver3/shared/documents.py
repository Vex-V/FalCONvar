...
Imports nothing, like `paths.py`, and that is load-bearing rather than tidy.
If a document's shape lived inside the component that produces it, `cut.py`
would have to import `boundaries` to read a timeline -- growing exactly the
import edges the file-handoff design exists to remove. Every component imports
a shape from here; no component imports another.

The split against the components is:

    documents.py        what a thing IS       Timeline, Media, Produced;
                                              as_dict / from_dict, fingerprint,
                                              index_at
    boundaries/grid.py  what a thing DOES     uniform(), from_cuts(), enforce()

`Timeline.index_at()` is shape -- given these spans, which one holds this
second. `enforce(min_s, max_s)` is derivation, and only that side knows that
policies exist at all.

These dataclasses are also the single source of truth for `db/json/*.schema.json`,
which is generated from them rather than maintained beside them.
"""
